...
////////////////////////////////////////////////////////////
bool Projectile::update(double dt, std::vector<sf::Sprite> & wallSprites, sf::Sprite & tankAI, bool & hit)
{
	if (!inUse())
	{
	...
	else 
	{
		...
		for (sf::Sprite const & sprite : wallSprites)
		{
			...
			//This if situation is used to check for collisions with the tanks
			if (CollisionDetector::collision(m_projectile, tankAI) && hit != true && m_speed != 0)
			{
				m_speed = 0;
				hit = true;
			}
...
{	
	return position.x - m_projectileRect.width / 2 > 0.f
		&& position.x + m_projectileRect.width / 2 < ScreenSize::s_width//width()
		&& position.y - m_projectileRect.height / 2 > 0.f
		&& position.y + m_projectileRect.height / 2 < ScreenSize::s_height;//height();
}